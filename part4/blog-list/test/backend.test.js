...
beforeEach(async () => {
  ...
  const promises = initialBlogs.map(b => new Blog(b).save())
  await Promise.all(promises)
})

test('APT correctly returns data in json format with status code 200',
  async () => {
...
test('Deleting a non-existing blog post resource', async () => {
  await api.delete(`/api/blogs/5a422bc61b54a676234d17f7`).expect(204)
})
